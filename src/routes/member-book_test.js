...
router.get('/edit/:sid', async (req, res) => {
        const sql = "SELECT * FROM `member_book` WHERE sid=?";;
        const [result] = await db.query(sql, [req.params.sid]);
        // res.redirect('/address-book/list');
        if (result.length) {
                result[0].birthday = moment(result[0].birthday).format('YYYY-MM-DD');
                res.render('member-book/edit', { row: result[0] });
        }
        else {
                res.redirect('/member-book/list');
        }
...
router.post('/edit', async (req, res) => {
        ...
        const sql = "UPDATE `member_book` SET ? WHERE sid=?";
...
router.get('/del/:sid', async (req, res) => {
        const sql = 'DELETE FROM `member_book` WHERE sid=?';
        const [result] = await db.query(sql, [req.params.sid]);
        // res.redirect('/address-book/list');
        if (req.get('Referer')) {
                res.redirect(req.get('Referer'));
